-- ============================================================================
-- MIGRATION PHASE 2 (INTÉGRALE, CORRIGÉE & SÉCURISÉE)
-- Date: 2026-09-21
-- Tables sanctuarisées : profiles, renovation_projects, renovation_briefs, auth.users
-- ============================================================================

-- 1. VÉRIFICATION DU TYPE ENUM user_role
DO $$
DECLARE
v_enum_exists BOOLEAN;
v_has_particulier BOOLEAN;
v_has_entreprise BOOLEAN;
BEGIN
SELECT EXISTS (
...
SELECT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'user_role' AND e.enumlabel = 'particulier') INTO v_has_particulier;
SELECT EXISTS (SELECT 1 FROM pg_enum e JOIN pg_type t ON e.enumtypid = t.oid WHERE t.typname = 'user_role' AND e.enumlabel = 'entreprise') INTO v_has_entreprise;

IF NOT (v_has_particulier AND v_has_entreprise) THEN
RAISE EXCEPTION 'MIGRATION ARRÊTÉE : Le type public.user_role existe mais les valeurs "particulier" et "entreprise" sont manquantes.';
END IF;
ELSE
...
-- 2. ENRICHISSEMENT ADDITIF DE public.profiles
...
-- 3. MISE À JOUR DE handle_new_user() (IDEMPOTENTE)
...
-- 4. ENRICHISSEMENT ADDITIF DE public.renovation_projects
ALTER TABLE public.renovation_projects ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'draft';
ALTER TABLE public.renovation_projects ADD COLUMN IF NOT EXISTS completion_percentage INT NOT NULL DEFAULT 0 CHECK (completion_percentage >= 0 AND completion_percentage <= 100);
...
-- -----------------------------------------------------------------------------
-- POLICIES : CONSULTATION_DOCUMENTS
-- -----------------------------------------------------------------------------
